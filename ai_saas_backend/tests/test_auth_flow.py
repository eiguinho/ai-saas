...
def test_login_success(test_client):
    resp = test_client.post("/api/users/login", json={
        "username": "testuser",
        "password": "Senha123!"
    })
...
def test_login_invalid_password(test_client):
    ...
    assert resp.get_json()["error"] == "Usuário ou senha inválidos"
...
def test_logout_and_token_revocation(test_client):
    ...
    # Logout
    logout_resp = test_client.post("/api/users/logout", headers=headers)
    assert logout_resp.status_code == 200

    # Usa token logout para acessar rota protegida
    access_resp = test_client.get(f"/api/users/{resp.get_json()['user']['id']}", headers=headers)
...
def test_access_with_revoked_token(test_client):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    res = test_client.get(f"/api/users/{resp.get_json()['user']['id']}", headers=headers)
